...
그림 4는 $\varepsilon$의 크기에 따른 $\varepsilon$-greedy 알고리즘의 퍼포먼스를 그래프로 나타내어 서로 비교한 것이다. 구체적으로, 임의의 10-armed bandit 2000개를 생성한 후 각 $0.1$-greedy, $0.01$-greedy 그리고 $0$-greedy 할고리즘으로 각각 1000회의 time step 동안 선택을 하도록 한 평균이 그래프에 표시되어 있다.먼저 어떤 알고리즘을 이용하던 간에 action 선택의 증가함에 따라 누적된 reward의 평균이 증가하고 일정 시점이 지나면 안정화됨을 알 수 있다.
