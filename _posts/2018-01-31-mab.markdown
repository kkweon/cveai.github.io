...
슬롯머신의 레버를 당기면  기대값이 0이고 분산이 1인 정규분포(Gaussian distribution, normal distribution)에 따라 숫자를 토해내는 이상적인 상황을 생각해보자. 여기서 평균이 0인 분포를 따른다는 것은, 이 슬롯머신의 레버를 당기는 시행을 반복하면서 얻게 되는 숫자들의 평균을 계산해보면 그 시행 횟수가 많아질수록 평균이 점점 0에 가까워지리라는 것이라는 것을 뜻한다. 
