...
먼저 가치함수 $Q_t(a)$를 구체적으로 정의해보자. 한가지 굉장히 자연스러운 선택은 $a$라는 action을 취하여 받은 reward의 평균을 그 함수값으로 정하는 것이다:  
$Q_t(a) := \frac{{\rm 시간} \, t \, {\rm 이전까지} \, a{\rm를 선택하여 얻은 reward의 합}{시간 t 이전까지 action a를 취한 횟수}$
