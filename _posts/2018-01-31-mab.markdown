...
이제까지의 내용을 종합해서 agent를 어떠헥 프로그래밍 할 것인지 그 한가지 방법을 자세히 살펴보자. $varepsilon$-greedy 알고리즘이다. 우리가 agent에게 설정해야할 프로그램의 내용은 각 시점 $t$에서 몇번 슬롯머신을 선택할 지에 관한 것이다. (수학적으로는 $A_\dot: \{0, 1, 2, \ldots\} \rightarrow \{a_1,a_2, \ldots ,a_{10}\}$ 이라는 함수를 정의하는 것과 같은 의미다.)
