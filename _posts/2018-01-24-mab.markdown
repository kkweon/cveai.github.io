...
자, agent를 통해 우리가 할 수 있는 일이란 각 시간 $t$에 하나의 action $A_t$를 취해서 그에 따르는 reward $R_t$를 받아 확인 하는 것이 전부다. 그렇다면 슬롯머신드르이 reward 분포를 알아내기 위해서 최초에 해야할 일은 일단 아무거나 혹은 마음에 드는 번호의 머신을 골라 당겨보는 것일 것이다. 그로 인해 받게 되는 reward 값이 해당 분포의 기대값과 얼마나 차이나는지를 알 수 는 없지만 그래도 어느정도 가깝울 거라고 기대해 볼 수는 있을 것이다.
